  ...
  return (
    <Tabs
      ...
      <Tabs.Screen
        ...
        options={{
          title: 'Cart',
          tabBarIcon: ({ color, focused }) => <IconSymbol size={28} name="cart" color={focused ? colorScheme === 'dark' ? 'white' : '#641691' : color} />,
          tabBarActiveTintColor: '#641691',
          tabBarLabel: ({focused, color}) => <ThemedText ignore={true} style={{color: focused ? color : color, fontSize: 12, lineHeight: 13}}>{translate ? 'Cart' : 'Cart'}</ThemedText>
        }}
      />
      <Tabs.Screen
        name="profile"
